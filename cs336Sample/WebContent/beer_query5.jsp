...
<% 
	...
	try{
		...
		ArrayList<Map<String,Integer>> list = new ArrayList();
   		...
   		String beer4 = request.getParameter("beer5");
   		//Make a query
   		String query = "select month(b.date), sum(t.quantity) from bills b, transactions t where t.bill_id = b.bill_id and t.item = \'" + String.format("%s", beer4) + "\' group by month(b.date) order by month(b.date) ASC";
   		//out.print(query);
   		...
   		while (result.next()) { 
   			...
   			map.put(result.getString("month(b.date)"),result.getInt("sum(t.quantity)"));
   			list.add(map);
   	    } 
   	    result.close();
        ...
        chartTitle = "Time distribution per month of the year for" + String.format("%s", beer4);
