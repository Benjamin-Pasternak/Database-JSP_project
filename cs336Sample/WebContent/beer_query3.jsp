...
<% 
	...
	try{
		...
		ArrayList<Map<String,Integer>> list = new ArrayList();
   		...
   		String beer3 = request.getParameter("beer3");
   		//Make a query
   		String query = "select Hour(b.time), sum(t.quantity) from bills b, transactions t where t.bill_id = b.bill_id and t.item = \'" + String.format("%s", beer3) + "\' group by Hour(b.time) order by Hour(b.time) desc";
   		//out.print(query);
   		...
   		while (result.next()) { 
   			...
   			map.put(result.getString("Hour(b.time)"),result.getInt("sum(t.quantity)"));
   			list.add(map);
   	    } 
   	    result.close();
        ...
        chartTitle = "Time distribution per time of day for" + String.format("%s", beer3);
